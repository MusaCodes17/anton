...
- **INV-6 · Deal qualification (B9-v2 + mx):** a deal exists iff `msrp IS NOT NULL` and the price is at least `MIN_DEAL_DISCOUNT_PCT` (default 5%, env-tunable) below MSRP — a minimum-discount **floor**, not merely `price < msrp`, so the ubiquitous `$X.99`-vs-round-`$X.00` penny artifacts don't render as 0%-OFF deals; savings measured against MSRP → `scrapers/orchestrator.py` `MIN_DEAL_DISCOUNT_PCT` (via `DealStore`) → `tests/test_orchestrator.py` (threshold boundary + penny-artifact + deep-discount cases) + `tests/test_deals.py` (retirement/requalification, orphan guard, promo rules remain uncovered; refactor.md H1/H2).
...
- **INV-9 · Single Uvicorn worker:** exactly one worker must be running at all times — D4's in-process scrape lock and E8's in-process rate limiter each assume a single process; multiple workers give each its own lock state and rate-limit bucket, silently breaking both invariants → pinned in `backend/Dockerfile` CMD (`--workers 1`) and `docker-compose.yml` → no automated test; enforced by deployment config — verify in `docker ps` / `uvicorn` logs on any restart. If multiple workers are ever needed, redesign D4/E8 first (see docs/design_decisions.md).
