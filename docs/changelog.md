...
- **[CHANGED] `components/layout/Layout.jsx` + `index.html` — header no longer hides under the status bar, and can't scroll off (Task 1):** root cause was **not** the `sticky`/full-bleed logic R5.1 touched — it was that nothing insets the top. `index.html`'s `apple-mobile-web-app-status-bar-style="black-translucent"` + `viewport-fit=cover` means iOS standalone paints web content *under* the notch and the app must inset the top itself via `env(safe-area-inset-top)`; nothing did, so the `h-16` header sat ~50px behind the Dynamic Island on every route. **1a:** `pt-[env(safe-area-inset-top)]` on the shell wrapper (banner + header both clear the notch; no-op at 0 inset). **1b:** converted the shell from `min-h-[100dvh]` document-scroll + `sticky top-0` header to a **fixed-height app shell** — outer `flex h-[100dvh] flex-col`; offline banner + mobile header are static `shrink-0` children *outside* the scroll region (dropped `sticky top-0` — the header can't scroll off because it isn't inside the scroller); `<main>` is now the single scroll container for all routes (`min-h-0 flex-1 overflow-y-auto`). This collapsed the full-bleed special case: `/assistant` is just `flex-1 min-h-0` and `ChatPage`'s own `h-full` fills it — the old `h-[calc(100dvh-4rem)] md:h-[100dvh]` magic is gone. Mobile slide-down menu is now a static `shrink-0` sibling above `<main>` (was `sticky top-16`). Grepped `pages/` for `sticky` — only `Layout.jsx` used it, so nothing depended on viewport-sticky resolution moving into `<main>`. Stale `index.html` comment (claimed `env(safe-area-inset-*)` was "used in index.css" — it never was) rewritten to point at the actual `Layout.jsx` insets.
- **[CHANGED] `pages/Deals.jsx` + `components/ShoeProductCard.jsx` + `components/ColorwaySelector.jsx` + `index.css` — two shoes per row on mobile with a density pass (Task 2):** groups grid `grid-cols-1 sm:grid-cols-2 lg:grid-cols-3` → **`grid-cols-2 lg:grid-cols-3`** (2-up from the smallest breakpoint, 3-up on `lg`). The grid swap alone shipped a cramped card, so: price `text-2xl` → `text-xl sm:text-2xl`; card inner padding `p-4` → `p-3 sm:p-4`; `ColorwaySelector` row `flex flex-wrap` → **`flex min-w-0 gap-[7px] overflow-x-auto scrollbar-none`** (a horizontally-scrollable strip clipped to the card, so the colorway/"?" tiles never bleed past the card's right edge at half width; buttons keep `shrink-0`). New `.scrollbar-none` utility in `index.css` (`@layer utilities`, Firefox + WebKit). **Beyond the brief but required by its "no text overflow" acceptance:** the in-stock badge was being clipped by the card's `overflow-hidden` at ~165px (the price block + badge in `justify-between` didn't fit) — the price/badge row is now `flex flex-wrap … gap-x-2 gap-y-1.5` with `min-w-0` on the price block and `shrink-0` on the badge, so the badge wraps below the price instead of overflowing.

**[VERIFIED]** `npm run build` clean (SW + manifest emitted; pre-existing chunk-size warning only). Backend suite **443 passing** (`backend/venv/bin/pytest -q`; unchanged — no backend touch). **Chromium verification (in-app browser, 390×844, DPR 2)** against the dev server + live backend, all green: static header on `/`, `/deals`, `/training`, `/shoes` (header box unchanged — top stays 0 — after scrolling `<main>` to its bottom, proving it doesn't scroll away); reachable nav from `/assistant` (header visible, toggle reveals the mobile nav, tapping Home routes to `/`); groups grid computes to **2 columns at 390px / 3 at 1280px**; `document.scrollWidth === clientWidth` (no horizontal overflow) on every route — confirms the colorway strip and badge no longer bleed; chat composer visible and not covered on `/assistant`. **Honest limitation:** Chromium reports `env(safe-area-inset-*)` as **0**, so automation cannot verify the Task-1a notch inset — that is a by-hand check on the installed iOS home-screen app after deploy. Playwright's own pinned Chromium refused to install on this macOS 13 host, so the equivalent Chromium-automation surface was used to run the same DOM/computed-style assertions. **Deploy:** frontend only — build on the Mac, `rsync dist/` → `/var/www/anton/`, `chmod -R a+rX`; then **remove + re-add the PWA to the home screen** (or at minimum force-close and reopen) — a lingering service worker / cached shell will otherwise mask the header fix, and the top inset only takes effect on a fresh standalone launch. Decision recorded as design_decisions **E13** (supersedes R5.1's header handling).

---

## RA2.2 (R5.1) — mobile UI tightening pass — 2026-09-04

**[CHANGED] Real phone use of the installable PWA surfaced four desktop-first layout bugs; all four are CSS/layout, none native-only — so R5.1 resolves toward "tighten the PWA," not "go native" (native stays parked). Frontend only: no backend, no migration, no serializer touch. One commit per task, `ra2:` prefix. `vite build` clean; backend suite 443 passing (unchanged — no backend touch). 380px-first (CLAUDE.md §5); `100dvh` + `env(safe-area-inset-*)` throughout since the app draws under the notch/home indicator.**

- **[CHANGED] `components/layout/Layout.jsx` — full-bleed chat no longer swallows the mobile header (Task 1, the "can't get back" bug):** the outer wrapper is now a flex column (`flex min-h-[100dvh] flex-col`); the offline banner and mobile header are non-shrinking children (`shrink-0`), so the `md:hidden` top bar with the hamburger is always reachable on `/assistant`. `<main>` for the full-bleed branch is sized to the space left under the 4rem header — `h-[calc(100dvh-4rem)] md:h-[100dvh]` (definite height, so ChatPage's `h-full` inner scroll regions resolve) — instead of the old `h-screen` that began below the sticky header and overflowed the viewport. Padded routes get `flex-1`.
- **[CHANGED] `components/layout/Layout.jsx` — FAB clearance (Task 4):** the non-full-bleed page wrapper gains `pb-[calc(6rem+env(safe-area-inset-bottom))]` (reset by `sm:p-6` on wider screens), so the last Deals card and its "% OFF" badge clear the `fixed bottom-6` Son-of-Anton FAB and the home indicator on the phone. No padding on the full-bleed branch (the FAB is hidden there — `ChatDrawer` returns null on `/assistant`).
- **[CHANGED] `pages/ChatPage.jsx` — mobile chat layout (Task 2):** root `h-screen` → `h-full` (fills the `<main>` Layout now sizes, killing the double-100vh that was half of Task 1's bug). The 280px conversation aside is `hidden md:flex`; on mobile the list is a `PanelLeft`-triggered slide-over (backdrop + left panel, `md:hidden`) so the thread gets the full 380px. List markup extracted to a shared `ConversationPanel` so desktop aside and mobile slide-over can't drift. Header restructured to stay one row at 380px (mobile "Conversations" button, truncating model-name span `max-w-[36vw] sm:max-w-none`). Composer gains `pb-[calc(1rem+env(safe-area-inset-bottom))]` to clear the home indicator.
- **[CHANGED] `pages/Deals.jsx` — collapsible filter deck on mobile (Task 3):** the six-filter `<Card>` is wrapped in a disclosure collapsed by default on mobile, always-expanded inline on `md+` (`Card` class `md:block` + conditional `hidden`) — mirroring the existing `Watching` collapse pattern. A `md:hidden` "Filters" toggle shows an active-filter count (new `activeFilters` count, `hasFilters` now derives from it); all existing filter state and the Clear-filters affordance are unchanged. Phone now shows header → compact Filters row → shoes immediately.

**[VERIFIED]** `npm run build` clean (SW + manifest emitted; pre-existing chunk-size warning only). Backend suite **443 passing** (unchanged — no backend touch). On-device visual acceptance (header reachable on `/assistant`, hamburger returns to Home/Deals; chat thread full-width with reachable list; Deals filters collapsed with shoes above the fold; last card + badge clear the FAB; composer above the home indicator; no horizontal scroll at ~380px) is the human step post-deploy. **Deploy:** frontend only — build on the Mac, `rsync dist/` → `/var/www/anton/`, `chmod -R a+rX`; then **force-close and reopen the installed PWA** so `autoUpdate` swaps the service worker (a stale SW looks like the fix didn't land). **Out of scope (noted, not built):** raw `## Rotation Status` headings in chat replies (a chat *output* formatting issue — blank-line-before-headings nudge, separate item); drawer-vs-`/assistant` redundancy (both remain; unifying is a separate design call).

---

## mx — minimum-discount threshold for deal qualification — 2026-09-04

**[CHANGED] Deal qualification now requires a minimum discount off MSRP, not merely "strictly below MSRP." Retailers price at `$X.99` against round `$X.00` MSRPs, so nearly every in-stock product qualified by one to five cents and rendered as a "0% OFF" deal (Puma Deviate Nitro Pure $189.99 vs $190.00; Mizuno Hyperwarp Pro $309.99 vs $310.00; norda 005 $324.95 vs $325.00). One commit. No migration; does not touch shoe/run/deal serialization (no T1/T2 precursor). Suite 438 → 443 passing. INV-6 amended (minimum-discount floor); INV-1/INV-9, C9, A4 intact.**

- **[CHANGED] `scrapers/orchestrator.py` — minimum-discount floor at the qualification chokepoint:** new module constant `MIN_DEAL_DISCOUNT_PCT = float(os.getenv("MIN_DEAL_DISCOUNT_PCT", "5.0"))`. `scrape_retailer_for_shoe` now qualifies a deal only when `shoe.msrp` is set, `details['price']` is truthy, and `(shoe.msrp - price) / shoe.msrp * 100 >= MIN_DEAL_DISCOUNT_PCT` (boundary inclusive). The `and is_stocked` conjunction (D8) and the `else` → `deactivate_deal` retirement path are unchanged, so sub-threshold deals retire automatically on the next scrape — **no migration, no backfill**. Docstrings updated on `scrape_retailer_for_shoe` and `DealStore.upsert_deal` (which no longer asserts merely "price < msrp"). Deliberately placed at the single chokepoint, **not** as a downstream filter in `services/deals.py` or the frontend — the R3.2 deal-alert agent and the MCP `get_deals` tool read the `deals` table directly, so a serving-layer filter would leave sub-threshold rows for them to surface.
- **[CHANGED] tests — `test_orchestrator.py` (11 → 16):** new `mx` section — one-cent-below-MSRP is no deal (penny artifact); exactly 5.0% off is a deal (inclusive boundary, `savings_percent == 5.0`); well-above-threshold unchanged; an active deal whose price rises to sub-threshold retires via the `else` branch; a deep single-size clearance ($189.99 vs $310 MSRP, one size in stock) still qualifies (guards the withdrawn-#4 case). Module docstring + the exactly-at-MSRP comment updated off the old "strictly below" wording. No existing case needed a value change (none had asserted a marginal-penny deal).

**[VERIFIED]** Backend suite **443 passing** (`backend/venv/bin/pytest -q`; was 438, +5). No frontend change. **Why 5%:** it separates the two cases the earlier #4 investigation conflated — the scraper reports the cheapest in-stock variant (`_money(pick_min=True)`), so a genuine ~39%-off single-size clearance still qualifies while the `.99`-vs-`.00` artifacts do not. **Deploy:** backend only — `git pull` + `docker compose up -d --build`, then trigger one scrape (the `else` branch retires sub-threshold deals each scrape, so no cleanup script). Tune via `MIN_DEAL_DISCOUNT_PCT` if 5% proves wrong. Decision recorded as design_decisions **B9-v3**; INV-6, domain_model §4.1, and the withdrawn-#4 changelog conclusion corrected.
...
> **Correction (2026-09-04, superseded by the `mx` entry below):** the "$189.99" item was indeed a genuine single-size clearance, but the broader conclusion that the ~45 on-sale count was *mostly* legitimate was **partly wrong**. A large share were `$X.99`-vs-round-`$X.00` **penny artifacts** (0%-OFF "deals"). INV-6 *did* need to change — a minimum-discount floor (`MIN_DEAL_DISCOUNT_PCT`, default 5%) was added in the `mx` session. Don't re-derive "the count is fine."
...
## RA2.2 — Progressive Web App pass (installable + offline READ; writes stay online-only) — 2026-08-31

**[ADDED] Anton is now an installable PWA: home-screen icon, standalone launch, offline app-shell + last-loaded-data reads. Writes deliberately require connectivity — no offline write-queue (deferred to a gated R5.x item, `RA2_2_PWA_PLAN.md` §6) because replaying confirmation-gated mutations against stale state would break INV-1 (mileage ledger) / INV-9 (single writer) / C9 (confirmation gate). Frontend-only over the already-remote, already-authed RA2.1 SPA plus a two-matcher Caddy header touch; no backend code, no new endpoint, no schema, no migration. Backend suite unchanged at 420 passing; `vite build` clean (SW + manifest emitted). Auth model (RA2.1 session cookie), transport auth, and every invariant untouched.**

- **[ADDED] `frontend/vite.config.js` — `vite-plugin-pwa` (`registerType: 'autoUpdate'`):** web manifest (`name/short_name` "Anton", `display: standalone`, `orientation: portrait`, `start_url`/`scope` `/`, `theme_color`/`background_color` `#0e0f11` from the `--background` token); Workbox precache of the built shell (`globPatterns` over js/css/html/svg/png/ico/fonts, 20 entries); `navigateFallback: /index.html` mirroring Caddy's `try_files`, with a denylist for `/api`, `/mcp`, `/health`, `/authorize`, `/token`, `/revoke`, `/oauth`, `/.well-known` so a navigation never resolves the SPA shell over a backend route; `cleanupOutdatedCaches` + `clientsClaim` + `skipWaiting` so an `autoUpdate` deploy doesn't strip-mine storage. `devOptions.enabled: false` (SW off in `vite dev`). **Runtime cache (§3 offline READ):** a single `NetworkFirst` entry scoped to **same-origin GET `/api/*`** (5s network timeout, 100-entry / 7-day cap, `statuses:[200]`) — the auth session endpoint `/api/auth/session` is **explicitly excluded** so no session/credential state is ever cached.
- **[ADDED] PWA icons — `frontend/public/icon.svg` (source) + generated `pwa-{64,192,512}.png`, `maskable-icon-512x512.png`, `apple-touch-icon-180x180.png`, `favicon.ico`:** generated by `@vite-pwa/assets-generator` via `frontend/pwa-assets.config.js` (`minimal-2023` preset, all variants `padding: 0`, green `#16a34a` background). The source SVG is the existing favicon mark (brand green `#16a34a` + `#0a0f0d` strokes) scaled to a full-bleed 512 square with the mark inset within the maskable safe zone — so every variant renders edge-to-edge with no white letterboxing. `npm run generate-pwa-assets` regenerates. **The mark is still the placeholder favicon glyph** — upgrade when the real Anton logo lands (known backlog item).
- **[ADDED] `frontend/index.html` — iOS PWA meta/link tags:** `apple-touch-icon`, `apple-mobile-web-app-capable`/`mobile-web-app-capable`, `apple-mobile-web-app-status-bar-style: black-translucent`, `apple-mobile-web-app-title`, `theme-color #0e0f11`, and `viewport-fit=cover` (iOS ignores most of the manifest; these are the install/standalone hints on the runner's actual Safari platform).
- **[ADDED] `frontend/src/lib/queryClient.js` — React Query client + IndexedDB persistence (§3):** the query cache is persisted to IndexedDB (`@tanstack/query-async-storage-persister` + `idb-keyval`, no localStorage ceiling) so a cold offline launch shows last-loaded rotation/deals/training instead of skeletons. `gcTime` raised to 7 days (an entry GC'd from memory drops from the snapshot); `dehydrateOptions.shouldDehydrateQuery` persists **successful** queries only and never `auth*` keys. **`clearOfflineData()`** wipes in-memory + IndexedDB + the `anton-api-reads` SW cache. `main.jsx` now uses `PersistQueryClientProvider` and calls `clearOfflineData()` on the app-wide `anton:unauthenticated` event (logout + any mid-session 401).
- **[ADDED] `frontend/src/hooks/useOnline.js` — real connectivity signal:** `navigator.onLine` + a `HEAD /health` no-store probe (`/health` is backend-routed, never SW-cached, so the check hits the real network), re-checked on `online`/`offline` events and foreground resume. Drives the offline indicator; the hard write-enforcement is the axios guard below.
- **[ADDED] `frontend/src/components/pwa/OfflineIndicator.jsx` + wired into `Layout.jsx`:** a sticky warning-token banner ("Offline — showing last synced data. Changes are paused until you reconnect.") shown only when offline — honesty over the illusion of live data. Renders above all sticky chrome; safe-area-inset padding for standalone iOS.
- **[CHANGED] `frontend/src/services/api.js` — axios write-guard (§4):** a request interceptor rejects any mutating method (POST/PUT/PATCH/DELETE) with "You're offline — this needs a connection." when `navigator.onLine === false`, **before** it fires — nothing is silently queued and no confirmed action commits later against stale state (protects INV-1/INV-9/C9). GETs pass through so the offline-read caches can serve. `frontend/src/hooks/useChatStream.js` gets the same guard inline (chat send is a `fetch()`, not axios, so it bypasses the interceptor).
- **[CHANGED] `frontend/src/components/auth/AuthGate.jsx` — offline-launch handling (§3):** a cold offline launch can't reach the `GET /api/auth/session` probe; since the httpOnly session cookie is still on the device, a probe failure **while offline** now enters the app optimistically (so the persisted cache is readable) rather than bouncing to login — a real 401 on the next online request still drops back to login. A genuine logged-out response (`200 {authenticated:false}`) instead calls `clearOfflineData()` before showing login. Without this, offline launch would be unusable (the whole point of §2/§3).
- **[CHANGED] `deploy/Caddyfile` — PWA static-serving headers (§5):** `Cache-Control: no-cache` on `sw.js` / `registerSW.js` / `workbox-*.js` (the SW script must always revalidate so deploys propagate; Workbox/Vite still revision the precache) and `Content-Type: application/manifest+json` on `manifest.webmanifest`. Verified the RA2.1 `@backend` matcher does not swallow these paths (none match `/api|/mcp|/health|/authorize|/token|/revoke|/oauth|/.well-known`).
- **[ADDED] deps:** `vite-plugin-pwa`, `@vite-pwa/assets-generator` (dev); `@tanstack/react-query-persist-client`, `@tanstack/query-async-storage-persister`, `idb-keyval`.

**[VERIFIED]** Backend suite **420 passing** (`backend/venv/bin/pytest -q`; no backend change). `npm run build` clean — 20 precache entries (1.15 MiB), `dist/sw.js` + `dist/workbox-*.js` + `dist/manifest.webmanifest` + `registerSW.js` emitted, manifest lists all four icon entries incl. maskable, index.html carries the injected `<link rel=manifest>` + register-SW script + iOS tags. `vite preview` + `curl -I`: `sw.js` → `text/javascript`, `manifest.webmanifest` → `application/manifest+json`, `pwa-512x512.png` → `image/png`, all 200. **Remaining acceptance is on-device (needs the deployed HTTPS host):** deploy (`git pull && docker compose up -d --build` + Caddy reload), then on the runner's iPhone Safari — "Add to Home Screen" installs with the icon, launches standalone (no browser chrome); airplane-mode cold launch shows last-loaded rotation/deals/training with the offline banner; every write control cleanly refuses offline with no queued side effect on reconnect; `curl -I` the SW on prod shows `Cache-Control: no-cache`. No frontend console-error check was possible headless — verify 0 console errors during the on-device pass.
...
- **[CHANGED] `backend/app/services/oauth.py` — `get_static_client()` sets `token_endpoint_auth_method`:** `"none"` when no secret is configured (public PKCE client → SDK's `ClientAuthenticator` takes its `== "none"` branch and skips secret validation, since `client_secret` is `None`), `"client_secret_post"` when a secret IS set (confidential path unchanged). Commented with the *why* (leaving it `None` is what raised "Unsupported auth method: None").
- **[CHANGED] `backend/app/main.py` — advertise `"none"` in AS metadata:** the SDK's `build_metadata` hard-codes `token_endpoint_auth_methods_supported` to the two confidential methods only. Rather than fork that logic, we rebuild the metadata via the SDK's own `build_metadata`, append `"none"` (to both `token_endpoint_auth_methods_supported` and, when present, `revocation_endpoint_auth_methods_supported`), and swap **only** the `/.well-known/oauth-authorization-server` route via `MetadataHandler` + `cors_middleware`. `/authorize`, `/token`, `/revoke` (SDK) and the RFC 9728 `/.well-known/oauth-protected-resource` route (`routers/oauth.py`) are left exactly as produced.
- **[ADDED] `backend/tests/test_oauth_public_client.py` (+5):** no secret → `token_endpoint_auth_method == "none"` and `client_secret is None`; secret set → `"client_secret_post"` and the secret round-trips; `ClientAuthenticator.authenticate_request` accepts a public-client `/token` POST (client_id only, no `Authorization` header, no `client_secret`) without raising; `GET /.well-known/oauth-authorization-server` lists `"none"` (and keeps the two confidential methods). Secret-sensitivity handled with `monkeypatch` because `load_dotenv()` reloads `ANTON_OAUTH_CLIENT_SECRET` from the live `.env` at import time.
- **[FIXED] `backend/tests/test_oauth.py::test_token_path_is_public` — pre-existing failure (unrelated to this feature; failed identically on clean HEAD, flagged in the RA2.1 entry below).** The installed `mcp[cli]` SDK answers a missing `client_id` with `401 unauthorized_client` (an OAuth-layer response), so the old `assert status != 401` was wrong. Rewritten to assert its true intent — the request reached the OAuth handler (JSON body with an `"error"` field) rather than being stopped by `BearerAuthMiddleware` (empty-body 401 with `WWW-Authenticate: Bearer realm="Anton"`).

**[VERIFIED]** Full suite **420 passing** (`backend/venv/bin/pytest -q`, was 419+1-failing). Acceptance checked end-to-end through the real ASGI `/token` route with the secret blanked (`ANTON_OAUTH_CLIENT_SECRET=""`): a public-client `authorization_code` exchange with a dummy code returns **`invalid_grant`** (NOT `unauthorized_client`), and `GET /.well-known/oauth-authorization-server` returns `token_endpoint_auth_methods_supported = ["client_secret_post","client_secret_basic","none"]`. No schema change, no migration, no frontend change (no `vite build` needed). **Deploy:** `git pull && docker compose up -d --build`, set `ANTON_OAUTH_CLIENT_SECRET=` (empty) in `backend/.env`, restart, then **delete and re-add** the claude.ai connector with the secret field BLANK (stale client state reproduces the failure).
