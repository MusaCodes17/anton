...
**Anton is internet-facing (RA1, deployed on Hetzner) with three independent, layered credential types — design_decisions E9 (RA1.1/RA1.1b/RA1.1c) and E10 (RA2.1).** All three are checked in one app-wide **pure-ASGI** middleware (`app/middleware/auth.py`) — pure-ASGI so SSE and the `/mcp` Streamable-HTTP stream pass through untouched — in this order: named bearer → OAuth 2.1 access token → session cookie. This superseded R2.1's single shared `ANTON_SECRET` (E7, LAN-trust model) once the goal became "reachable from Claude mobile anywhere," which requires a public HTTPS endpoint and per-client revocation that a shared cleartext token can't provide.

**1. Named bearer tokens (`ANTON_TOKENS="name:token,..."`).** Each REST/MCP client — `desktop` (Claude Desktop via `mcp-remote --header`), `loopback` (Son of Anton's internal MCP client, scoped so the secret never reaches an external server) — has its own revocable token, compared with a constant-time, no-short-circuit OR across all entries. The `spa` token was **retired in RA2.1** (see credential 3).

**2. OAuth 2.1 + PKCE, for the claude.ai connector (web + mobile).** claude.ai's connector calls Anton from **Anthropic's cloud infrastructure**, not from the user's device — the reason the endpoint must be public HTTPS at all (a Tailscale/LAN overlay cannot satisfy this). Implementation (`services/oauth.py`, `AntonOAuthProvider`, `routers/oauth.py`): a full OAuth 2.1 authorization server (`mcp[cli]`'s SDK plumbing) with a password-gate login page (`ANTON_LOGIN_PASSWORD`, `secrets.compare_digest`), 256-bit auth codes and tokens stored as SHA-256 hex, 1-hour access / 30-day refresh tokens rotated on use. As of **RA1.1c**, Anton registers as a **public PKCE client** (`token_endpoint_auth_method="none"` when no client secret is configured) rather than requiring a shared confidential secret — the standard, lower-maintenance posture for a single personal connector; a confidential-client path (`client_secret_post`) remains available if a secret is set. Discovery requires **both** `/.well-known/oauth-authorization-server` (RFC 8414, SDK-provided) **and** `/.well-known/oauth-protected-resource` (RFC 9728, added in `routers/oauth.py` — the connector fetches this *first* to learn which authorization server protects the MCP resource; omitting it makes discovery fail silently against whatever the Caddy fallback route serves).

**3. Session cookie, for the SPA (RA2.1).** The SPA is served publicly (`https://anton.musasouled.com`, via Caddy `file_server` over the built `dist/` — see the Caddy split below) and authenticates with an httpOnly `anton_session` cookie issued by `POST /api/auth/session` after a password check against the same `ANTON_LOGIN_PASSWORD`. This retired the baked-in `VITE_ANTON_SECRET` — the built bundle now carries **no credential at all**, closing the RA1 non-goal of shipping a full-access token in a publicly downloadable JS file. Cookies introduce CSRF exposure that bearer auth doesn't: mutating `/api/*` requests authenticated by the cookie require `Origin == ANTON_HOST_URL` (exact match) in addition to `SameSite=Lax`; bearer- and OAuth-authenticated requests carry no cookie and are exempt; GET/HEAD/SSE are always exempt.

**The Caddy split (RA2.1, `deploy/Caddyfile`, live on the host at `/etc/caddy/Caddyfile` — the repo copy is a template, not the running config):** an explicit `@backend` matcher (`/api`, `/api/*`, `/mcp`, `/mcp/*`, `/authorize`, `/token`, `/revoke`, `/oauth/login`, both `.well-known` OAuth paths, `/health`) reverse-proxies to Uvicorn with `flush_interval -1` (unbuffered, streaming-safe) and forwarded headers; everything else falls through to `file_server` over the built SPA with `try_files {path} /index.html` for client-side routing — **no basic_auth**, since the login page + cookie is the gate. Uvicorn is launched with `--proxy-headers --forwarded-allow-ips="*"` (`entrypoint.sh`) so it honors Caddy's `X-Forwarded-Proto: https`; without this, Starlette's own trailing-slash redirects downgrade to `http://`, which OAuth-aware clients correctly refuse to follow (a real bug hit and fixed during RA1.5 cutover).

Facts a due-diligence review must state plainly:

- **Public allowlist (no token):** `GET /`, `GET /health`, `GET /api/health`, both `.well-known` OAuth discovery paths, `/authorize`, `/token`, `/revoke`, `/oauth/login`, `POST/GET/DELETE /api/auth/session` (each validates internally — safe to expose; can't require the cookie it's about to issue), the SPA's static assets (served by Caddy, never by this backend), and all CORS `OPTIONS` preflight. Everything else — every destructive REST endpoint, the scrape triggers, the **MCP write tools**, and `POST /api/chat/message` (the paid-LLM proxy) — requires one of the three credentials above.
- The server **binds loopback inside the container**; Caddy is the only public listener, terminating TLS and reverse-proxying to `127.0.0.1:8000`. The app **fails fast** at startup if `ANTON_TOKENS` is unset (`require_auth_config()`).
- Secrets live in `backend/.env` on the **server** (never committed, never synced from a laptop `.env`) — gitignored. The SPA bundle now carries **zero** secrets (RA2.1); provider API keys still never ship to the frontend (the chat proxy pattern holds, unchanged since R2.1).
- Every 401 is logged at WARNING with source IP; per-IP rate limiting applies to both auth failures (`auth_failure_limiter`) and login attempts (`login_failure_limiter`), returning 429 + `Retry-After` after the burst (RA1.3).
...
**Deliberately not (yet) covered:** rate limiting is per-IP and in-process (single-worker, INV-9 — a second worker would need a shared store); OAuth/session token revocation is DB-row deletion, not a hot-rotation UX; Dynamic Client Registration is disabled (the connector is one statically-registered client in `.env`, by design — no self-registration surface).

Net: the security architecture is now "public internet, single tenant, three layered credential types, no shared secret ships in any client artifact." This is the substrate every further exposure-increasing step (a second MCP client such as ChatGPT, a native mobile app) builds on — adding a client generally means extending the static OAuth client registry, not re-architecting auth.
...
1. ~~**Zero authentication across three mutation surfaces**~~ **RESOLVED (R2.1, 2026-07-07; superseded by RA1.1/RA1.1b/RA1.1c and RA2.1).** Three layered credential types now gate all mutation surfaces — named bearer tokens, OAuth 2.1 for the claude.ai connector, session cookie for the SPA — see §11 and design_decisions E9/E10. TLS is live (Caddy, Let's Encrypt) and the SPA carries no baked secret. The item is kept here (struck through) rather than deleted so the "this dominated everything" history stays visible.
