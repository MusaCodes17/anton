...
**Chosen:** The platform is **Anton** (assistant: **Son of Anton**). The **in-code product strings** — FastAPI title, root message, chat `SYSTEM_PROMPT`, the COROS-sync agent prompt, the MCP server name, the READMEs, and `package.json` — were flipped to **Anton** in R1 (2026-07-14); the **GitHub repo and local folder** were renamed to `anton` in R2/R3 (2026-07-14). Only the **`shoe_deals.db` filename** deliberately retains the old name.
**Why:** Renaming the DB filename mid-flight is churn with near-zero user value and real risk — the Litestream replica path keys off the DB filename. The in-code strings, the repo, and the folder had no such coupling (a repo rename is a redirect; a folder rename is a `mv`), and the identity was user-visible.
**Advantages:** Zero breakage; identity now reads "Anton" everywhere a human or connector sees it, while the one load-bearing path name (the DB file) stays stable.
**Trade-offs:** Mild residual mismatch that the on-disk DB file is still `shoe_deals.db` — mitigated by the glossary (`domain_model.md` §7.1) and the comment on `DATABASE_URL`.
**Verdict:** 🔁 **Superseded — rename executed 2026-07-14** (`MAINTENANCE_PLAN.md` §4). R1 (in-repo strings), R2 (GitHub repo → `MusaCodes17/anton`), and R3 (local folder → `anton`) are all done; the local git remote was repointed to the new URL. The only surviving retained name is the `shoe_deals.db` filename, kept for the Litestream coupling above.
...
### E13. Standalone-iOS safe-area top inset + fixed app shell with an internal-scroll `<main>` (RA2.2 R5.2 — corrects R5.1's header handling)

**Chosen (2026-09-10):** the mobile app shell is a **fixed-height flex column** (`flex h-[100dvh] flex-col`) whose header and offline banner are **static `shrink-0` children outside the scroll region**, and whose `<main>` is the **single scroll container** for every route (`min-h-0 flex-1 overflow-y-auto`). Two coupled fixes to two real-phone bugs:
- **Top safe-area inset (1a).** `index.html` sets `apple-mobile-web-app-status-bar-style="black-translucent"` with `viewport-fit=cover`. Under iOS standalone that means *paint web content under the status bar; the app must inset the top itself via `env(safe-area-inset-top)`.* Nothing did — so the `h-16` header rendered at `y=0` with ~50px behind the Dynamic Island on every route (the "header missing/unreachable" report). Fix: `pt-[env(safe-area-inset-top)]` on the shell wrapper (both banner and header clear the notch). The bottom inset was already handled on `<main>` (`env(safe-area-inset-bottom)` for FAB/home-indicator clearance) — this makes the top symmetric. A **no-op wherever the inset is 0** (desktop browsers, non-notched devices), so it's safe everywhere.
- **Fixed shell over document-scroll + `sticky` header (1b).** R5.1 kept a `min-h-[100dvh]` document-scroll column with a `sticky top-0` header and a `h-[calc(100dvh-4rem)]` special-case height for the full-bleed chat route. `position: sticky` inside a scrolling flex column **intermittently fails in iOS standalone mode** (the header scrolls off), which is the fragile path this replaces. Moving the header *out of* the scroll container makes it structurally unable to scroll away — no `sticky` needed. It also collapses the full-bleed special case: with an internal-scroll `<main>`, the `/assistant` branch is just `flex-1 min-h-0` and `ChatPage`'s own `h-full` fills it; the `h-[calc(100dvh-4rem)] md:h-[100dvh]` magic (a symptom of the document-scroll approach) is gone.

**Why:** `black-translucent` is an explicit contract that the app owns the top inset; leaving it unhandled is a latent bug that only manifests on an installed standalone launch (never in the browser, never in Chromium emulation — which reports `env(safe-area-inset-*)` as 0). And a scroll-owning `<main>` is the robust way to pin chrome on iOS, versus `sticky`, which the platform honours inconsistently in standalone PWAs.
