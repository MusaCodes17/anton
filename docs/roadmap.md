...
| RA1.5 | **Cutover & validation** – 🟡 **substantially done (2026-08-29); one step remains** | Execute the plan §7 runbook: E4-style count reconciliation across the DB move, re-point Claude Desktop, add the claude.ai connector. Two exit criteria: (1) **end-to-end mobile sync on cellular** — COROS fetch → suggest → confirm → run lands with rich fields and mileage updates; (2) **the DC-IP scrape checkpoint** — full scrape from the new host compared per-retailer against the home baseline via R2.5 `scrape_runs`; material degradation invokes the home-box escape hatch (no paid-bypass escalation, D3 stands). **Criterion (2): ✅ done (2026-08-29)** — compared Hetzner-scraped counts against the pre-cutover baseline per retailer; every enabled retailer within normal week-to-week noise, JD Sports Canada (the canonical bot-flag risk, Shopify) flat at 39 products, zero `warning`/`error` health states, zero watchdog alerts. **No degradation — Option A (Hetzner) confirmed; the home-box escape hatch is not invoked.** **Criterion (1): connector auth chain ✅ fixed end-to-end (RFC 9728 discovery → RA1.1c public-PKCE client → the HTTPS→HTTP downgrade-redirect fix, see changelog "RA1.6" entry 2026-08-29), claude.ai connects and lists tools — but the actual validation run (COROS fetch → suggest → confirm → run lands with rich fields, executed *off WiFi on the phone*) has not yet been performed.** This is the one remaining human step closing RA1.5. Also outstanding from RA1.3/RA1.4: the restore drill and the uptime pinger (human steps, never executed). | The milestone's definition of done is the user story itself, plus proof the deals domain didn't quietly pay for it. | RA1.1–RA1.4 | Medium |
| RA1.6 | **Docs reconciliation** – 🟡 **partially done (2026-08-29)** | `architecture.md` §11 rewritten for the internet trust model ✅ done (2026-08-29 — now describes the three-credential-type model, RFC 9728, the Caddy split, and the proxy-headers fix, not just the superseded shared-token model); `CLAUDE_DESKTOP_SETUP.md` ✅ rewritten (2026-08-29 — both Desktop bearer-token and claude.ai OAuth connector setup, plus the three known-fixed failure modes documented so they aren't re-diagnosed from scratch). **Still open:** A1 formal amendment note in design_decisions.md (RA1.5 hosting is already recorded inline on A1, but hasn't been re-read against the final cutover state); E7 → per-client-tokens is already superseded by E9/E10 in design_decisions.md (done, just needs a final read-through once RA1.5's remaining step closes); R5.2 formally closed as pulled-forward-and-executed (currently only noted informally in the roadmap dependency spine); `CLAUDE.md` INV-9 presence unverified this session. | A fresh session reading `docs/` must describe the deployed reality, not the laptop. | RA1.5 | Low |
...
| R4.2 | **Agent scheduling & event delivery** *(blocked on R3.5 — deferred with it)* | Weekly summary auto-runs Monday morning; deal alerts fire on new-deal events from scheduled scrapes; both deliver via R3.5's channel. | Turns R3's on-demand agents into ambient value — Anton starts *telling* the runner things. | R3.1 ✅, R3.2 ✅, R3.5 ⏸, R4.1 | Medium |
| R4.3 | **Semi-automated COROS cadence** *(blocked on R3.5 — deferred with it)* | Within the Claude-Desktop-mediated constraint (design_decisions C6): a "runs pending sync" nudge (Home strip already shows last-sync) + one-tap launch of the `sync_coros_runs` flow. Full automation is *not* possible while COROS OAuth is desktop-managed — design to the constraint, don't fight it. | Sync friction is the main data-freshness gap on the training side. | R3.5 ⏸ (nudge channel) | Low–Medium |
