...
| # | Item | Description | Why it matters | Dependencies | Complexity |
|---|---|---|---|---|---|
| R4.1 | **Scheduled scraping** ✅ **Done (2026-07-10)** | Shipped: `services/schedule.py` — `AsyncIOScheduler` with `CronTrigger` (default 3am America/Toronto); opt-in via `SCRAPE_SCHEDULE_ENABLED=true`; `SCRAPE_SCHEDULE_CRON` for override. Wired in `main.py` lifespan. In-memory scrape lock deliberately kept (APScheduler `max_instances=1` + the existing `threading.Lock` = at most one scrape at a time; D4 resolved). `ScrapeRun.trigger="scheduled"` for history. `GET /api/admin/schedule` exposes status + last 5 scheduled runs. SettingsSync "Scheduled Scraping" card (React Query poll 60s). 9 tests; suite 298 → 307. APScheduler 3.x (not 4.x — different API) re-admitted (R1.6). | Deal data is only as good as its freshness; manual triggering is the platform's biggest remaining chore. | R2.5 (observability) ✅, R1.6 decision ✅; R2.1 before exposing any trigger surface ✅ | Medium–High |
