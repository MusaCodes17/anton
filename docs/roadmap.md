...
| R4.5 | **Scraper watchdog** *(blocked on R3.5 — deferred with it)* | Trend rules over R2.5 data: "retailer X has returned 0 products for 3 runs" → alert via R3.5. | Silent scraper death is the current failure mode (visible only in logs); the deal feed degrades invisibly. | R2.5 ✅, R3.5 ⏸, R4.1 | Low |

**Order within R4:** 4.1 → 4.4 → 4.5 → 4.2 → 4.3. **Reordered 2026-07-10:** 4.1 and 4.4 need nothing from R3.5 and lead; 4.5/4.2/4.3 wait behind the R3.5 deferral (see R3 section above) — building them first would mean a "scheduled" job that writes output nobody sees, no better than the on-demand tools R3.1–R3.4 already ship.
