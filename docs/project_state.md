...
| Test suite | ✅ **172 passing** (+1 xfailed), 26 modules (Session N: `test_deal_store.py` +9, `test_orchestrator.py` +5 & +1 xfail — deal retirement/requalification, orphan non-empty guard, promo manual-protection, MSRP truth table, H2 gap captured as xfail; Session M: `test_chat_history.py` +6, `test_checkpoints.py` +3; Session L: `test_scrape_history.py` +8; Session K: `test_shoe_types.py` +8; Session J: `test_rate_limit.py` +5; Session I: `test_activities_union.py` +1 composed filter+pagination; Session H: `test_races.py` +2, `test_activity_tags.py` +18, `test_activities_union.py` +1 (PB activity_id); Session G: `test_fitness.py` +2, `test_activity_edit.py` +6, `test_activities_union.py` +1; Session F: `test_activity_tags.py` +3, `test_activities_model.py`/`test_activities_union.py` +2/+4; `test_auth.py` +13 Session D) |
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) is now shipped** — R1 + C1/M3 safety fixes, R2.1 (security) + R2.2 (schema authority) live, R2.7 (training depth, T1–T8), R2.3 (indexed reads) + rate limiting (E8) + R2.4 (shoe-type vocabulary) + R2.5 (scrape observability) + **R2.6 (chat/memory persistence)**. The ⚠️ scheduled-to-change decision list is empty. The deal-domain test gaps are now closed (Session N). **Next is R3 (AI capabilities), starting with R3.1 (Weekly Rotation Summary Agent)** — the deal domain R3 agents lean on now has retirement/requalification, orphan-guard, and promo-protection coverage.

0. **[FOLLOW-UP] Live browser visual pass on R2.6** — the desktop + ~380 px pass on the chat page and the checkpoint prompt wasn't done because the dev backend on :8000 was unresponsive this session. Do it once the backend is back (build is clean, HTTP layer verified end-to-end; nothing structural is unverified).
1. ~~**Deal-domain test gaps**~~ ✅ **Closed (Session N, 2026-07-08)** — `test_deal_store.py` (+9: retirement/requalification round-trip, orphan non-empty guard B10, promo manual-beats-scraped D6) and `test_orchestrator.py` (+5: MSRP qualification truth table + orphan-on-rename, via injected stub scraper). The **H2 partial-failure gap** is captured as an `xfail` asserting the desired behaviour (it flips to a pass when H2 is fixed) — H2 itself remains documented debt (refactor.md H2). The one open piece of refactor.md H1 is the `TestClient` HTTP-layer smoke slice (still no route-level tests).
