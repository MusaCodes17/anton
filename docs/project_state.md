...
**Snapshot date:** 2026-07-14 (after Defect Block B session — D1: FK pragma + `rotation.delete_owned_shoe()` + 10 tests; D3: COROS dedup confirmed; D4: 3 code hazard fixes; D5: P2.3 sparkline confirmed not built; D6: env-leak fix. Suite 352 → **362**. Prior: Defect Block A. **All of R2 + RA1.0–RA1.4 (code) + R3.1–R3.4 + R3.6 + R4.1 + R4.2 + R4.4 + Defect Block A + Defect Block B now shipped; RA1.5 cutover is the blocking human task.**)
...
**Where things stand right now:** the multi-phase **Anton redesign is functionally complete** — all five tabs (Home / Training / Shoes / Deals / Son of Anton) are built. The most recent R4 work: R4.2 Scrape Reliability shipped 2026-07-10 — retry in `BaseScraper._fetch_with_requests` (3 attempts, 2 s backoff) + watchdog (`_WATCHDOG_THRESHOLD=3` consecutive-failure detector, `retailers_needing_attention` in `scrape_health`) + 10 tests; suite 313 → **323** (322 passing). R4.4 Coupon Hunting and R4.1 Scheduled Scraping also shipped same day. **Documentation program** (`documentation_creation.md`) is **complete and committed** (R1.1, 2026-07-06). **All of R2 and RA1.0–RA1.4 (code) and R3.1–R3.4 + R3.6 + R4.1 + R4.2 + R4.4 are shipped.** RA1.5 (cutover) is the next human task; roadmap R4.2 (Agent scheduling) and R4.3 (COROS cadence) remain blocked on R3.5.
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 + RA1.3 + RA1.4 (code) + R3.1 + R3.2 + R3.3 + R3.4 + R3.6 + R4.1 + R4.2 + R4.4 + Defect Block A + Defect Block B are now shipped.** Suite: **372 passing**. RA1.5 cutover is the blocking human task; roadmap R4.2/R4.3 remain blocked on R3.5.
