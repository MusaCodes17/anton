...
**Snapshot date:** 2026-07-07 (after Phase 2 Session D — **R2.1 the security pass shipped**: shared bearer token on `/api`+`/mcp`, `127.0.0.1` default bind, loopback + SPA + Desktop all send the token; suite 75 → 88; design_decisions E1 → Superseded by E7. Live go-live is a documented human step — `CLAUDE_DESKTOP_SETUP.md`).
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R1 is done, the review's same-day safety fixes (C1 + M3) are done, and R2.1 (the security pass) has shipped** (2026-07-07); the exposure gate in front of R3–R5 is closed. Focus continues down R2.

0. **Go live on R2.1 (a human step, not code).** The auth code is committed and tested, but enforcement only starts when the server is restarted with the secret set. Do the `CLAUDE_DESKTOP_SETUP.md` §"Rollout order": set `ANTON_SECRET` in `backend/.env` and the same value as `VITE_ANTON_SECRET` in `frontend/.env`, update the Claude Desktop `mcp-remote --header`, then restart the backend and rebuild/hard-reload the SPA. Verify with its checklist. **Until this is done the running server is still the old, unauthenticated code.**
...
2. **R2.2 schema authority** (Alembic sole source; relocate DB/`.bak` out of the tree) — prerequisite hygiene before any table-adding work (R2.5/R2.6).
