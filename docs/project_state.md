...
**Snapshot date:** 2026-08-29 (after **RA1.6-partial** — the `--proxy-headers --forwarded-allow-ips="*"` fix on Uvicorn's launch, closing an HTTPS→HTTP downgrade-redirect bug that was the last blocker on the claude.ai connector's `/mcp` calls; `architecture.md` §11 and `CLAUDE_DESKTOP_SETUP.md` rewritten same day to reflect the OAuth 2.1 + session-cookie auth model (deploy-config + docs only — suite unchanged at 420). Also this session: **RA1.5 exit criterion (2) closed** — a full scrape from Hetzner compared per-retailer against the pre-cutover baseline showed no degradation (JD Sports Canada, the bot-flag canary, flat at 39 products; zero warning/error health states), confirming the always-on host (Option A) with no need for the home-box escape hatch. Prior: RA1.1c public/PKCE-only OAuth client (2026-08-12, suite 420) fixed `unauthorized_client — Unsupported auth method: None` at `/token`. Prior: RA2.1 SPA session-cookie auth (2026-07-20, suite 415) and the RFC 9728 protected-resource metadata commit. **All of R2 + RA1.0–RA1.4 (code) + RA1.1c + RA2.1 + R3.1–R3.4 + R3.6 + R4.1 + R4.2 + R4.4 + R4.6 + Defect Block A + Defect Block B now shipped; RA1.5's one remaining step is the actual off-WiFi mobile COROS-sync validation run — the auth chain that blocked it is now fully fixed.**)
...
| Test suite | ✅ **420 passing** (RA1.1c public PKCE OAuth client: `test_oauth_public_client.py` new +5, and the previously-flagged `test_oauth::test_token_path_is_public` failure is now fixed — it asserted the wrong thing about the SDK's missing-`client_id` 401; RA2.1 SPA session-cookie auth: `test_auth.py` +15 (session cookie accept/reject/expiry, CSRF Origin guard, session login/probe/rate-limit), `test_migrations.py` EXPECTED_TABLES += `sessions`; R4.6 New-Retailer Onboarding Agent: `test_onboarding.py` new +21; T5: `test_orchestrator.py` +2; maintenance batch 2026-07-14: `test_migrations.py` new +1 (T4), `test_owned_shoes.py` +4 (T3); per-shoe ScrapeRun (`0935675`): `test_scrape_history.py` +5; Defect Block B: `test_delete_owned_shoe.py` new +10; Defect Block A: `test_kids_filter.py` new +26, `test_orchestrator.py` +3; R4.2: `test_scraper_retry.py` new +4, `test_scrape_history.py` +6; R4.4: `test_coupon_hunter.py` new +8; R4.1: `test_schedule.py` new +9; R3.6: `test_race_advisor.py` new +14; R3.3: `test_shoe_review.py` new +7; R3.2: `test_deal_alerts.py` new +22; R3.1: `test_weekly_summary.py` new +20; RA1.3: `test_access_log.py` new +15; `test_auth.py` +5; `test_oauth.py` +3; RA1.1b: `test_oauth.py` new +18; RA1.1: `test_auth.py` rewritten +8 net; earlier sessions: see prior entries) |
...
- **No auth on three mutation surfaces** + default `0.0.0.0` bind (deliberate; gates all exposure). **The top open item; its plan now exists** (`docs/archive/SECURITY_PASS_PLAN.md`, ready to execute as R2.1).
...
-15. **RA1.1c — Anton is a public (PKCE-only) OAuth client for the MCP connector** (2026-08-12 — ✅ Keep; refines E9). `get_static_client()` sets `token_endpoint_auth_method` = `"none"` when no secret is configured (public PKCE client — the OAuth 2.1 / MCP-preferred posture for a single personal connector) and `"client_secret_post"` when a secret IS set (confidential path unchanged). Leaving it `None` made the SDK's `ClientAuthenticator` raise `unauthorized_client — Unsupported auth method: None`, blocking the token exchange. `main.py` also appends `"none"` to the AS metadata's `token_endpoint_auth_methods_supported` (the SDK's `build_metadata` hard-codes only the two confidential methods) by rebuilding the metadata and swapping only the `/.well-known/oauth-authorization-server` route. Transport-auth only — `/mcp`, RA2.1 session cookie, named bearers, and the RFC 9728 protected-resource route untouched. Deploy needs `ANTON_OAUTH_CLIENT_SECRET=` (empty) and a fresh connector re-add with the secret field blank.
-14. **E10 — SPA session-cookie auth as a third credential type** (RA2.1, 2026-07-20 — ✅ Keep; extends E9). The SPA authenticates with an httpOnly `anton_session` cookie issued after a password login (`POST /api/auth/session`), retiring the baked `spa` bearer so the built bundle carries no secret. `sessions` table (migration `1a2b3c4d5e6f`) + `services/sessions.py` mirror the OAuth-token hashing discipline. Auth middleware gains a third branch (named-bearer → OAuth → session cookie) plus a CSRF guard (`Origin == ANTON_HOST_URL`) that applies **only** to cookie-authenticated mutating `/api/*` writes — bearer/OAuth exempt, GET/SSE exempt, SameSite=Lax is the first line. Caddy now serves the built SPA (`file_server`, no basic_auth) and reverse-proxies only the enumerated backend paths. `/mcp` + the claude.ai connector untouched. See E10 in design_decisions.md.
-13. **D1 — SQLite FK enforcement + sanctioned shoe-delete path** (2026-07-14 — ✅ Keep). `PRAGMA foreign_keys=ON` set via SQLAlchemy event listener on every connection (was advisory before). `rotation.delete_owned_shoe()` is the sole write path for owned-shoe deletion: NULLs nullable FK refs (PlannedRace, StravaGearMapping), deletes CheckpointPrompts, preserves strava archive (INV-4). Live `PRAGMA foreign_key_check` confirmed clean before enabling. 10 tests.
-12. **D8 — out-of-stock qualification rule** (2026-07-14 — ✅ Keep). A deal requires BOTH below-MSRP AND in-stock; OOS products retire any existing deal and requalify automatically when stock returns. Shopify search `available` default flipped to `False` (pessimistic-until-detail-fetch). Touches same qualification path as D2 (already fixed bcdddc2). No design_decisions.md entry — the rule is a direct consequence of INV-6 honesty.
...
- **Convention** (docs/archive/REDESIGN_PLAN.md §5): one phase per Claude Code session; one commit per numbered task with phase-prefixed conventional messages (`p5: canonical activities migration`); backend endpoints land *with tests* before their consuming UI task; every phase ends suite-green + desktop & ~380 px visual pass.
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.1c + RA1.2 + RA1.3 + RA1.4 (code) + RA2.1 + R3.1 + R3.2 + R3.3 + R3.4 + R3.6 + R4.1 + R4.2 + R4.4 + R4.6 + Defect Block A + Defect Block B are now shipped.** Suite: **420 passing**. RA1.5 cutover is the blocking human task; roadmap R4.2/R4.3 remain blocked on R3.5.

0. ~~**RA1.1c — public (PKCE-only) OAuth client for the MCP connector**~~ ✅ **Done (2026-08-12)** — `get_static_client()` sets `token_endpoint_auth_method` (`"none"` public / `"client_secret_post"` confidential); AS metadata advertises `"none"`; fixes the connector's `unauthorized_client — Unsupported auth method: None` at `/token`. 5 new tests + 1 pre-existing failure fixed; suite 419+1-failing → **420**. Deploy: blank `ANTON_OAUTH_CLIENT_SECRET` + re-add the connector with the secret field empty.
...
0. ~~**R4.4 — Coupon Hunting Agent**~~ ✅ **Done (2026-07-10)** — `get_promo_page_urls` hook on `BaseScraper` (multi-page promo discovery, subclass-overridable); `services/coupon_hunter.py` (`get_stacking_opportunities` — read-only query returning retailers with both active promo codes AND active deals); `get_coupon_opportunities` MCP tool (stacking view) + `hunt_coupons` MCP tool (triggers `detect_all_promo_codes`, no scrape lock) + `coupon_digest` MCP prompt. 8 tests; suite 307 → 313.
0. ~~**R4.2 — Scrape Reliability**~~ ✅ **Done (2026-07-10)** — `_fetch_with_requests` retry (3 attempts, 2 s backoff, WARNING on retry, ERROR on exhaustion); `_derive_watchdog_alert` + `_WATCHDOG_THRESHOLD=3` in `scrape_history.py` (consecutive error/warning streak detector); `watchdog_alert`/`watchdog_reason` on every retailer entry; `retailers_needing_attention` top-level field in `scrape_health`. `scrape_health` MCP docstring updated. 10 new tests. Suite 313 → 323 (322 passing). Note: roadmap R4.5 (Scraper watchdog) absorbed here — watchdog alerts via existing `scrape_health` MCP tool, sidestepping the R3.5 dependency. Roadmap R4.2 (Agent scheduling) stays blocked.
0. ~~**Defect Block A (D8 + D7)**~~ ✅ **Done (2026-07-14)** — D8: OOS qualification guard (retire deal when `in_stock=False` or no sizes, even if below MSRP; Shopify pessimistic default); D7: composite kids filter (URL + name) + `_is_youth_size` excluding youth variants from price pool and sizes. D2 confirmed already fixed (bcdddc2, 2026-07-08). 29 new tests; suite 323 → 352. Two `mx:` commits.
0. ~~**Defect Block B (D1 + D3/D4 + D5 + D6)**~~ ✅ **Done (2026-07-14)** — D1: FK pragma on every SQLite connection + `rotation.delete_owned_shoe()` (NULLs nullable FK refs, preserves strava archive, 10 tests); D3: COROS dedup verified against live DB; D4: 3 hazard fixes (trigger_scrape dict bug, is_already_logged date type, promo sort TypeError); D5: P2.3 sparkline confirmed not built; D6: env-leak fix in test_schedule.py. Suite 352 → 362. Two `mx:` commits.
0. ~~**R4.6 — New-Retailer Onboarding Agent (MAINTENANCE_PLAN I1)**~~ ✅ **Done (2026-07-14)** — `services/onboarding.py` (queue detection + read-only `probe_retailer` + `apply_onboarding` single write path + `mark_unscrapable`); 4 MCP tools (`get_onboarding_queue`/`probe_retailer`/`onboard_retailer` confirm-gated/`mark_retailer_unscrapable` confirm-gated) + `retailer_onboarding` prompt; REST parity on `/retailers/*`; `scrape_health.needs_onboarding` shared with the R4.5 watchdog. 21 tests; suite 374 → 395.
1. **RA1.5 — Cutover & validation** — 🟡 **host provisioned, connector auth fully working, one step remains (2026-08-29).** Host deployed (Hetzner, `anton.musasouled.com`), Claude Desktop re-pointed, claude.ai connector added. **Exit criterion (2) — DC-IP scrape comparison — ✅ done (2026-08-29):** Hetzner-scraped counts vs. the pre-cutover baseline, per retailer, showed no degradation (JD Sports Canada flat at 39; zero warning/error states; zero watchdog alerts) — Option A confirmed, home-box escape hatch not invoked. **Exit criterion (1) — mobile sync E2E on cellular — auth chain ✅ fixed, validation run not yet executed:** three real bugs blocked the connector end-to-end (missing RFC 9728 protected-resource metadata → RA1.1c public-PKCE client → HTTPS→HTTP downgrade-redirect from Uvicorn not trusting Caddy's forwarded headers, fixed 2026-08-29 via `--proxy-headers --forwarded-allow-ips="*"` in `entrypoint.sh`). claude.ai now connects and lists tools. **What's left: actually run the COROS fetch → suggest → confirm → log flow from the phone, off WiFi**, and confirm the run lands with rich fields + mileage updates — this has not yet been executed. Also still outstanding from RA1.3/RA1.4: the restore drill and the uptime pinger (human steps, never executed). `REMOTE_ACCESS_PLAN.md` §6/§7 is the runbook.
...
4. **Optional follow-ons:** ~~the shoe-major synchronous scrape doesn't yet emit R2.5 `scrape_runs`~~ ✅ **Done (2026-07-14, commit `0935675`)** — per-shoe scrapes now write `ScrapeRun` rows (`trigger="shoe-sync"`, excluded from health/watchdog, included in `recent_runs`); +5 tests. Remaining: watchlist reduction still O(N) Python in `services/watchlist.py` (labelled, fine — leave it).

---

*Maintenance note: this file describes 2026-07-14 (Defect Block B — D1 FK pragma + sanctioned shoe delete + D3/D4/D5/D6 hazard fixes) and decays fastest of all the docs. Update the Snapshot date, §2 table, §9, and §11 at session end; move shipped items from §4/§5 into §3. When in doubt, the `docs/changelog.md` top entries are the source of truth for what happened; this file is the source of truth for what it means.*
