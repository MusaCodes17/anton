...
**Snapshot date:** 2026-07-08 (after Phase 2 Session L — **R2.5 scrape observability**. New `scrape_runs` table (migration `d0e1f2a3b4c5`) persists one row per retailer per full-catalog scrape attempt (status/counts/error), written only by `ScrapeOrchestrator.scrape_retailer` (stamped `running` up front, finalized `success`/`error`). `services/scrape_history` derives per-retailer health (`ok`/`warning`=quietly-broken/`error`/`unknown`); `GET /api/scrape/history` + MCP `scrape_health` serve it; Settings → Sync & Scraping gains a "Retailer health" card. Wired into the background `/scrape/all` + synchronous `/scrape/retailer/{id}` flows. design_decisions **D8** (+ D5 verdict updated; single-process lock D4 deliberately unchanged). Suite 141 → **149**. Prior: Session K **R2.4 shoe-type vocabulary** (migration `c9d0e1f2a3b4`, suite → 141); Session J **chat rate limiting** (E8, → 133); Session I **R2.3 indexed reads + watchlist service**; Session H **R2.7 T7–T8 + Training polish**; Sessions F/G **R2.7 T1–T6**; Session E **R2.2**; Session D **R2.1**. **R2.1–R2.5 + R2.7 + rate limiting done; next active R2 item is R2.6 server-side chat/memory persistence.**)
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **R1 done; C1/M3 safety fixes done; R2.1 (security) + R2.2 (schema authority) shipped and now *live*; R2.7 (training depth) complete (T1–T8); R2.3 (indexed reads + watchlist service) + chat rate limiting (E8) + R2.4 (shoe-type vocabulary) + R2.5 (scrape observability) shipped.** The active R2 thread is now R2.6, then the deal-domain test gaps.

1. **R2.6 server-side chat/memory persistence** — move Son of Anton conversations + checkpoint state off localStorage into the backend (design_decisions C8, the last ⚠️ scheduled-to-change item; depends on R2.1, now done).
2. **Deal-domain test gaps**: retirement/requalification, the orphan guard + its H2 partial-failure gap, promo manual-beats-scraped (refactor.md H1/H2).
