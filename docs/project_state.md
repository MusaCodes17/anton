...
**Snapshot date:** 2026-07-09 (after RA1.1b session — **OAuth 2.1 connector auth**: `AntonOAuthProvider` + 2 new tables + login page + 18 new tests; capability-URL code deleted; suite 194 → **210**. Prior: RA1.2 — Deployment substrate (`backend/Dockerfile` + `docker-compose.yml` + `deploy/Caddyfile` + INV-9). Prior: RA1.0 + RA1.1 — research spikes, D0 hosting decision, named per-client tokens. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 are now shipped; RA is the active milestone; R3/R4 are ⏸ parked.**)
...
| Test suite | ✅ **210 passing**, 28 modules (RA1.1b: `test_oauth.py` new +18; `test_auth.py` −4 capability-URL tests; net +14; RA1.2: stable; RA1.1: `test_auth.py` rewritten +8 net; R2.7.2: `test_races.py` +2; Session Q: +3; Session O: +13; Session N: +15; Session M: +9; Session L: +8; Session K: +8; Session J: +5; earlier sessions: see prior entries) |
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 are now shipped.** Suite: **210 passing**. RA is the active milestone; R3/R4 are ⏸ parked.

0. ~~**RA1.1b**~~ ✅ **Done (2026-07-09)** — OAuth 2.1 (`AntonOAuthProvider` + login page + 18 tests, capability-URL deleted). Suite 194 → 210.
1. **RA1.3 — Surface & abuse hardening** — 401 logging with source IP, in-process or firewall-level auth-failure throttle, structured access log; credential redaction in all logs (Caddyfile log filter is already written — verify it covers OAuth auth code / state params once deployed); uptime monitoring on `/health`. `REMOTE_ACCESS_PLAN.md` §6 RA1.3.
2. **RA1.4 — Backups off-laptop** — Litestream continuous replication (preferred) or nightly snapshot to runner-controlled object storage; restore drill performed before cutover counts as done. `REMOTE_ACCESS_PLAN.md` §6 RA1.4.
3. *(after RA1.3 + RA1.4)* **RA1.5 — Cutover & validation** — provision host, deploy container, E4 count reconciliation, re-point Claude Desktop, add claude.ai connector; two exit criteria: mobile sync E2E on cellular + DC-IP scrape comparison via R2.5 `scrape_runs`.
4. *(after RA1)* **R3.1 Weekly Rotation Summary Agent** — first proactive agent (roadmap R3, order 3.1 → 3.4 → 3.3 → 3.2 → 3.5 → 3.6).
...
6. **Optional follow-ons:** the shoe-major synchronous scrape doesn't yet emit R2.5 `scrape_runs` (deliberate deferral, D8); watchlist reduction still O(N) Python in `services/watchlist.py` (labelled, fine); `deals`/`dashboard` fat routers remain; surface chat 429 as client-side toast.

---

*Maintenance note: this file describes 2026-07-09 (RA1.1b session) and decays fastest of all the docs. Update the Snapshot date, §2 table, §9, and §11 at session end; move shipped items from §4/§5 into §3. When in doubt, the `docs/changelog.md` top entries are the source of truth for what happened; this file is the source of truth for what it means.*
