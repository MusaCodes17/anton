...
**Snapshot date:** 2026-08-12 (after RA1.1c — public/PKCE-only OAuth client for the claude.ai MCP connector: `get_static_client()` sets `token_endpoint_auth_method`, AS metadata advertises `"none"`; fixes `unauthorized_client — Unsupported auth method: None` at `/token`; 5 tests + 1 pre-existing failure fixed; suite **420**. Prior: RA2.1 SPA session-cookie auth (2026-07-20, suite 415) and the RFC 9728 protected-resource metadata commit. Earlier: R4.6 New-Retailer Onboarding Agent (suite 395). **All of R2 + RA1.0–RA1.4 (code) + RA1.1c + RA2.1 + R3.1–R3.4 + R3.6 + R4.1 + R4.2 + R4.4 + R4.6 + Defect Block A + Defect Block B now shipped; RA1.5 cutover is the blocking human task.**)
...
1. **RA1.5 — Cutover & validation** — provision host, deploy container, E4 count reconciliation, re-point Claude Desktop, add claude.ai connector; two exit criteria: mobile sync E2E on cellular + DC-IP scrape comparison via R2.5 `scrape_runs`. **Also during RA1.5:** restore drill + uptime pinger (RA1.3/RA1.4 remaining human steps). `REMOTE_ACCESS_PLAN.md` §6/§7 is the runbook. **Prep done (2026-07-14):** `deploy/reconcile.sh` automates the E4 count/head/integrity check (runbook §7 steps 1 & 5 now invoke it); deploy artifacts audited internally-consistent; live DB verified `integrity ok` + FK-clean at head `a2b3c4d5e6f7` (baseline 942/707/8,159.3/676). Remaining is all human infra: provision host, fill OAuth redirect-URI + B2 vars, deploy, run both validations.
