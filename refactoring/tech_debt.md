...
|---|---|---|---|---|
| **P0-1** | Writable `current_mileage`/`starting_mileage` via generic `PUT /owned-shoes/{id}` — the mileage-ledger invariant (§4.5/§4.6) has an undocumented parallel write path, blind-`setattr`, no server-side confirmation. The UI *deliberately* uses it (Adjust-mileage dialog), so the constitution and the code disagree. | Fragile areas (§11.1) | **Active** | refactor.md **C1** |
| **P0-2** | No auth on three mutation surfaces (REST, `/mcp`, chat LLM-proxy incl. the arbitrary-URI resource-read proxy) on a default `0.0.0.0` bind; CORS broader than needed. Priority preserved from refactor.md: land **before** any unattended agents. | Architectural (§1.1) | **Scheduled** (E1 ⚠️) | refactor.md **C2**; arch §15.1/§16.1 |
...
| **P1-5** | `shoe_type` free-string vocabulary — the load-bearing cross-domain join key — now exists in **four** places (backend strings, `lib/shoeTypes.js`, MCP docstrings, chat `SYSTEM_PROMPT`) with **no boundary validation** on `shoe_type` *or* owned-shoe `status`. A typo = silent feature degradation, never a 422. | Fragile areas (§11.5) | **Active** | refactor.md **M2**; B3 🕐; domain_model §4.3 |
...
|---|---|---|---|---|
| 1.1 | **No auth ×3 surfaces + `0.0.0.0` default bind** | **P0** | Scheduled (E1 ⚠️) | Ledger rank P0-2. The hard gate in front of every growth direction (mobile, remote MCP, agents-unattended). Detail: refactor.md C2, arch §16.1. |
| 1.2 | **Dual schema authority** (`create_all` + Alembic + retained `legacy_migrations/`) | **P1** | Scheduled (A6 ⚠️) | Ledger rank P1-9. `legacy_migrations/` archival rides this sweep (dead_code §2.2). |
...
| 1.7 | **APScheduler declared, unused** — a dependency without an architecture, inviting drive-by wiring that collides with 1.3's lock assumptions. | P2 | Scheduled (E5 ⚠️) | Decide deliberately: design (with persisted job state, arch §16.6) or drop (dead_code §8). |
...
| 5.4 | **`scraper_manager.py` compat shim** — pure re-exports; real orchestrator/lock/registry boundaries invisible at all four call sites; "temporary" is aging. | P2 | Scheduled (D7 ⚠️) | One mechanical sweep (dep_graph §11.2); batch with Task D. *Transition scaffolding with live consumers — never dead code* (dead_code §5 note). |
| 5.5 | **Pace formatting ×3** (`rotation`, `coros_client`, the `ShoeRun.avg_pace` proxy) — one formatting rule, three copies that can drift, one of them in the model layer. | P2 | Active | dep_graph §8.3/§11.3. The pure `pace` module is the cheapest fix on this entire ledger and unblocks §5.3's shrink. |
...
| 5.7 | **Hard-coded model catalog in `routers/chat.py` vs prefix routing in `chat_service`** — two files encode one fact. | P2 | Active | dep_graph §8.5/§11.9; folds into §5.2's fix. |
...
| 6.3 | **Task D leftover: `coros_sync → owned_shoes._attach_computed_fields`** — private-helper import between sibling routers; response-shaping logic trapped in a router another router needs; also the near-cycle (§7.2). | P2 | Active | dep_graph §10.2/§11.1. One session; also collapses one of the three owned-shoe renderings (§5.1). |
...
| 6.5 | **All four scraper consumers coupled to the `scraper_manager` shim.** | P2 | Scheduled (D7 ⚠️) | Same item as §5.4, from the consumer side. |
...
| # | Item | Sev | State | Notes |
|---|---|---|---|---|
| 11.1 | **The writable mileage ledger** (`OwnedShoeUpdate.current_mileage` + blind setattr; `starting_mileage` edits don't delta the counter; two-step confirm exists only in the browser, not the API; `status` also pattern-unvalidated). The single most dangerous schema exposure — and an *undocumented invariant exception*, since the UI uses it deliberately. | **P0** | Active (ledger P0-1) | Full analysis + revised fix (sanctioned `rotation.adjust_mileage()`): refactor.md **C1**. |
...
| 11.7 | **Scrape-lock wedge** — setup code before the `try/finally` means one transient setup error holds the lock until process restart; every subsequent scrape 409s with no UI explanation. Low probability, standing outage when it hits. | P2 | Active | refactor.md **M3**: move the try up; same-day fix. |
...
- **The standing to-do list of decided change** is design_decisions' ⚠️ set — **E1 (auth), A6 (schema authority), C8 (chat memory), D7 (shim), E5 (APScheduler)** — all catalogued above with their ranks. These flip to Superseded entries as they're executed; strike the entries here in the same session.
