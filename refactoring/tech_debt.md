...
**Generated:** 2026-07-06, against the post-Phase-5 tree. **Freshness verified this session:** `models.py` at 18,151 bytes (canonical `Activity`, attribution-only `ShoeRun`); top of `docs/changelog.md` shows nothing newer than the 2026-07-04 Phase-5 entries. **Re-stamped 2026-07-06 evening (docs reconciliation):** tree now at the MSRP-drives-deals state (`models.py` 18,858 bytes · suite 64 · migration `d4e5f6a7b8c9`); the change touched deal qualification/savings paths only — P0/P1 rotation findings stand; **P1-4 narrowed** and **§9.5 struck**, **§9.7 resolved** below. **Re-stamped 2026-07-14 (T8 ledger reconciliation, suite 371):** struck the rows resolved since generation — **P1-3** (D1 FK pragma + sanctioned delete), **P1-5 `status`** (T3), **P1-9** (R2.2 schema authority), **P1-10 / §6.1** (fat-router extractions), **§8.3** (T4 migration test), **§9.1** (T6 façade), **§9.2** (CLAUDE.md §14 INVARIANTS); **§2.1** narrowed. **Re-stamped 2026-07-14 (T5, suite 374):** **P1-4 / §8.1** struck — the deal-domain invariant coverage refactor.md H1 asked for already existed (`test_deal_store.py`, 5cb5c56, 2026-07-08, missed in the T8 pass) and was extended today (target-ignored-in-qualification + price-requalify round-trip); the only residual is HTTP-layer tests for deal *endpoints* (§8.2).
...
| ~~**P1-3**~~ | ~~`DELETE /owned-shoes/{id}` bypasses rotation semantics… **and** SQLite FK enforcement is off process-wide.~~ ✅ **Resolved 2026-07-14 (D1):** `PRAGMA foreign_keys=ON` set via a SQLAlchemy connect listener (every connection); `rotation.delete_owned_shoe()` is the sanctioned delete path (NULLs nullable FK refs, deletes checkpoints, preserves the strava archive — INV-4); `DELETE` route now calls it. 10 tests in `test_delete_owned_shoe.py`. | Fragile areas (§11.4) | **Resolved** | refactor.md **H3** (struck) |
| ~~**P1-4**~~ | ~~Deal domain nearly untested — retirement/requalification, orphan guard, promo manual-beats-scraped uncovered.~~ ✅ **Deal-domain invariants covered** — `test_deals.py` (MSRP savings/refresh/no-MSRP), `test_deal_store.py` (5cb5c56, 2026-07-08: retirement, requalification, all three orphan-guard cases, promo manual-beats-scraped), `test_orchestrator.py` (MSRP truth table, price-rise retire, orphan, D2 partial-detail guard, D8 OOS retire/requalify, and 2026-07-14 T5: target-ignored-in-qualification + price-requalify round-trip). **Residual (separate item, §8.2):** no HTTP-layer tests for the deal *endpoints*. Migrations-never-exercised sub-gap resolved (§8.3, T4). | Missing tests (§8.1) | **Resolved** (invariants); §8.2 HTTP layer open | refactor.md **H1** (resolved) |
| **P1-5** | `shoe_type` free-string vocabulary — the load-bearing cross-domain join key. **`shoe_type` half resolved (R2.4, 2026-07-08):** single backend source (`app/utils/shoe_types.py`), served at `GET /api/shoe-types`, validated on the write schemas (off-vocab → 422); `lib/shoeTypes.js` reduced to presentation-only; MCP/chat-prompt mentions are descriptive text, not a value store. **`status` half resolved (T3, 2026-07-14):** `OWNED_SHOE_STATUSES` tuple + `validate_owned_shoe_status` on the owned-shoe write schemas (off-vocab → 422; read schemas unvalidated); 4 tests. | Fragile areas (§11.5) | **Resolved** (shoe_type R2.4; `status` T3) | refactor.md **M2** (struck); B3 🕐; domain_model §4.3 |
...
| **P1-8** | ~~Provider agentic-loop triplication in `chat_service` (~100 lines × 3) + model catalogs duplicated router-vs-service.~~ ✅ **Resolved 2026-07-10** — `BaseLLMProvider.run()` owns the shared loop; each provider implements 5 focused abstract methods (`_tool_schema`, `_check_configured`, `_prepare_messages`, `_stream_turn`, `_append_tool_results`). `_ToolCall` dataclass normalises cross-provider tool call representation. Model catalog was already single-sourced (R1.5d). | Poor abstractions (§5.2) | **Resolved** | dep_graph §9.6/§8.5; refactor.md H5; C2 🕐 |
| ~~**P1-9**~~ | ~~Dual schema authority: `create_all` at boot + Alembic.~~ ✅ **Resolved 2026-07-07 (R2.2):** Alembic sole authority (startup `alembic upgrade head`; `create_all` demoted to test fixtures; baseline recreates the pre-Alembic schema); `legacy_migrations/` deleted; DB + backups moved to `~/anton-data/`. A6 → Superseded. The FK pragma that "belonged in the same session" landed in D1 (see P1-3). | Architectural (§1.2) | **Resolved** (A6 → Superseded) | arch §15.5/§16.2 |
| ~~**P1-10**~~ | ~~Fat legacy routers (`watchlist`, `deals`, `dashboard`) with inline ORM — blocks MCP watchlist parity.~~ ✅ **Resolved:** `watchlist` → `services/watchlist.py` (R2.3, 2026-07-08, unblocking MCP `get_watchlist` in R3.4); `deals` → `services/deals.py` and `dashboard` → `services/dashboard.py` (2026-07-10), with MCP `get_deals`/`get_shoe_deals` calling the same service (REST/MCP parity). Residual: watchlist reduction stays O(N) Python in the service (labelled, fine at scale). | Tight coupling (§6.1) | **Resolved** | dep_graph §10.1/§11.5 |
...
| # | Item | Sev | State | Notes |
|---|---|---|---|---|
| 2.1 | **Mixed `app.models` façade vs `app.models.models` direct imports in the same files** (`shoes.py`, `owned_shoes.py`, others). *(The façade-incompleteness half is resolved — T6, 2026-07-14, see §9.1.)* Remaining: pick one import convention. | P2 | Active (narrowed) | dep_graph §11.4: complete the façade or drop it; one convention. |
...
| 2.4 | **Pre-rebrand names.** ✅ **In-code strings fixed (R1, 2026-07-14):** API title, root message, chat `SYSTEM_PROMPT` (the user-visible one — refactor.md L4b), COROS-sync prompt, MCP server name, READMEs, `package.json`. **Still retained:** repo name `running-shoe-deals`, folder path, DB filename — pending the GitHub + folder rename (R2/R3, at/around RA1.5). | P3 | Narrowed (E6 partially superseded) | MAINTENANCE_PLAN §4 R1; changelog 2026-07-14. |
...
| # | Item | Sev | State | Notes |
|---|---|---|---|---|
| 6.1 | ~~**Fat routers with inline ORM** (`watchlist`, `deals`, `dashboard`).~~ ✅ **Resolved** — all three extracted to services (`watchlist` R2.3; `deals`/`dashboard` 2026-07-10); MCP now exposes the watchlist (`get_watchlist`, R3.4). See ledger P1-10. | **P1** | Resolved | dep_graph §10.1/§11.5. |
...
The suite (**371 passing** as of 2026-07-14 — live count in the changelog/project_state) covers the rotation/training domain well — including, verified this session, the **retirement pipeline and replacement-deal heuristic at service level** (`test_rotation_overview.py` locks in the 75% threshold, worst-first ordering, and type-match heuristic). The gaps are concentrated and asymmetric:

| # | Item | Sev | State | Notes |
|---|---|---|---|---|
| 8.1 | ~~**Deal domain: nearly untested**~~ ✅ **Resolved** — the invariant coverage refactor.md H1 called for shipped in `test_deal_store.py` (5cb5c56, 2026-07-08) + `test_orchestrator.py` (D2/D8 + T5 2026-07-14); retirement/requalification, orphan guard, and promo manual-beats-scraped are all pinned. See ledger P1-4. | **P1** | Resolved | refactor.md H1 (resolved). |
...
| 8.3 | ~~**Migrations never exercised** — `conftest` builds schema via `create_all`.~~ ✅ **Resolved 2026-07-14 (T4):** `test_migrations.py` runs `alembic upgrade head` as a subprocess against a fresh tmp SQLite DB and asserts the load-bearing tables exist — the production boot path (R2.2) now has an automated check. | P2 | Resolved | — |
...
| # | Item | Sev | State | Notes |
|---|---|---|---|---|
| 9.1 | ~~**`models/__init__.py` façade missing the entire Phase-5 era**~~ ✅ **Resolved 2026-07-14 (T6):** `PlannedRace`, `StravaGearMapping`, `AthleteMetric`, `OAuthAuthCode`, `OAuthToken` added to the import + `__all__` (`Activity`/`ScrapeRun` were already present). *Residual:* the "exporting schemas nothing uses" audit is deferred (removing exports risks façade consumers; low value). | P2 | Resolved (export gap) | dep_graph §11.4; dead_code §4.5. |
| 9.2 | ~~**No INVARIANTS section anywhere** codifying domain_model §4 as checkable statements.~~ ✅ **Resolved (R1.1, 2026-07-06):** `CLAUDE.md §14` is the checkable INVARIANTS list (INV-1…INV-9), one line each with owning code path + covering test. | P2 | Resolved | CLAUDE.md §14. |
